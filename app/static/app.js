...
function renderDetail(m) {$('#detail-title').textContent=m.title;$('#detail-sub').textContent=m.provider?`ОНЛАЙН · ${m.provider.toUpperCase()}`:m.source==='room'?'ПЕРЕГОВОРНАЯ':'ЗАГРУЖЕННАЯ ЗАПИСЬ';$('#detail-meta').textContent=`${m.meeting_date} · ${m.participants.join(', ')||'Участники не указаны'}`;$('#detail-state').textContent=labels[m.state]||m.state;$('#detail-state').className=`state ${m.state}`;$('#detail-stage').innerHTML=`<b>${esc(m.stage)}</b>${m.error?`<p>${esc(m.error)}</p>`:''}`;$('#detail-content').hidden=!['review','approved','error'].includes(m.state);$('#approve').hidden=m.state!=='review';$('#export-pdf').hidden=$('#export-docx').hidden=m.state!=='approved';$('#retry').hidden=m.state!=='error';$('#export-pdf').href=`/api/meetings/${m.id}/export?format=pdf`;$('#export-docx').href=`/api/meetings/${m.id}/export?format=docx`;$('#detail-summary').textContent=m.summary||'Саммари ещё не сформировано.';$('#item-count').textContent=`(${m.items.length})`;$('#detail-items').innerHTML=m.items.length?m.items.map(item=>`<div class="item"><div class="item-top"><span class="tag ${esc(item.kind)}">${kinds[item.kind]||esc(item.kind)}</span><span class="state">${esc(item.status)}</span></div><h3>${esc(item.title)}</h3><p>${esc(item.description||'')}</p><small>Ответственный: ${esc(item.owner||'требует уточнения')} · Срок: ${esc(item.due_text||'требует уточнения')}</small><small>Источники: ${item.source_segment_ids.map(id=>`№${esc(id)}`).join(', ')}${item.review_note?' · Проверить: '+esc(item.review_note):''}</small>${m.state==='review'?`<div class="item-controls"><button data-edit-item="${esc(item.id)}">Изменить данные</button></div>`:m.state==='approved'&&item.kind==='action'?`<div class="item-controls"><button data-done="${esc(item.id)}">${item.status==='done'?'Вернуть в работу':'Отметить выполненным'}</button></div>`:''}</div>`).join(''):'<div class="empty">Элементов пока нет. При проверке добавьте пропущенное вручную через API.</div>';
